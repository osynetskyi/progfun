...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = 
    try {filter(tweet => tweet.retweets > elem.retweets).mostRetweeted}
  	catch {
...
object GoogleVsApple {
  ...
  def selectRelevant(source: List[String]): TweetSet =
    TweetReader.allTweets.filter(elem => source.exists(word => elem.text contains word))
  
  lazy val googleTweets: TweetSet = selectRelevant(google)
  lazy val appleTweets: TweetSet = selectRelevant(apple)
